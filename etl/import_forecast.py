...
Fonte (aponte via env RIVERFLOW_FORECAST_SOURCE se o layout for outro):
  <fonte>/live_ensemble.csv   code,horizon,date,q_median,q_p10,q_p90,trig_prob,trig_thr,name
"""
...
def main() -> None:
    ...
    for code, g in df.groupby("code"):
        ...
        metric = station["primary_metric"]

        points = []
        for _, row in g.sort_values("horizon").iterrows():
            points.append({
        ...
        # risco: maior prob. de cruzar o gatilho P95 em qualquer horizonte ate
        # 15d, e o pico (mediana) correspondente -- mesma definicao do
        # trigger_prob_15d do alert_latest.json do RIVERFLOW, recalculada
        # aqui pra cobrir TODAS as estacoes (o alert_latest.json so lista as
        # sinalizadas/top10, nao o painel inteiro)
        peak_row = g.loc[g["q_median"].idxmax()]
        risk_row = g.loc[g["trig_prob"].idxmax()]
        stations[code] = {
            "metric": metric,
            "trig_thr": round(float(g["trig_thr"].iloc[0]), 2),
    ...
    print(f"OK -> {OUT_PATH}  ({len(stations)} estacoes, emitido {issued})")
    if skipped:
        print(f"  ignoradas (fora do painel): {skipped}")
